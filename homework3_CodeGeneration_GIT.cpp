...
/// 1.函数的print去掉树的部分√
/// 2.将函数名改写为func√
/// 3.将表达式的print部分去掉树的部分，修改为value:type 冗余
/// 4.添加一个数据结构map<string,expr>来保存应当使用的expr,在函数构造成功时添加
/// 5.函数依然用dump，表达式用新函数show，利用已定义的函数进行计算时，树之间传递值要注意两个东西：类型和值，类型可以从Returntype获取。
